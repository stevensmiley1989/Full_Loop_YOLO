...
for Threshold in tqdm(COCO_iOUs):
    ...
    for PRED_i in tqdm(Predictions):
        ...
        if prediction_i and gt_i:
            ...
            df_i=df_i.drop_duplicates(subset='boxB',keep='first').sort_values(by='iou',ascending=False).reset_index().drop('index',axis=1)
            df_i_TP_index=list(df_i[df_i.iou>Threshold].index)
            df_i_FP_index=list(df_i[(df_i.iou<=Threshold)].drop_duplicates(subset='boxA',keep='first').index)
